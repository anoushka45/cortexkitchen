...
- **Cross-agent assumption diffing** — each domain node writes the assumptions it acted on into shared state; `EvaluationSanityChecker` cross-diffs them after the parallel fan-out and surfaces contradictions (e.g. menu assumed no stockouts while inventory flagged items low) as `stale_assumptions` injected into the critic's LLM prompt
