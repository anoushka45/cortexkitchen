"""Kindred replay endpoint (P6-A27).
...
input. So replay here means "make this one LLM call again with the same
provider/messages" — reusing BaseLLMProvider.complete(), the exact same
call path every node already uses, NOT re-running the 11-node graph. An
earlier version of this endpoint mistakenly ran the full pipeline (6+ LLM
calls) per replay when it didn't recognize the input shape — do not
