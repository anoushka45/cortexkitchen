...
_CACHE_KEY_PREFIX = "briefing_service:summary:"

_SUMMARY_SYSTEM_PROMPT = (
    "You write a 2-3 sentence executive summary for a restaurant owner opening their "
    "dashboard for the day. You are given their current health score, net margin, "
    "inventory shortages, and recent complaint themes -- all already computed, do not "
    "invent numbers not given to you.\n\n"
    "Write plain prose, no markdown, no bullet points, 2-3 sentences total. Lead with "
    "an overall read (solid/mixed/concerning), name the single most important number, "
    "then flag the one or two things that actually need attention today (a shortage, "
    "a complaint trend) if any exist. If nothing needs attention, say so plainly instead "
    "of inventing a concern. Specific beats generic -- name the actual ingredient or "
    "category, not \"some items\" or \"certain areas\"."
)
...
class BriefingService:
    ...
    async def _get_summary(self, org_id: int | None) -> Optional[dict]:
        ...
        critical_shortages = [r["text"] for r in risks if r["kind"] == "inventory"]

        facts = {
            "health_score": health_score,
            "net_margin_pct": net_margin_pct,
            "positive_sentiment_pct": positive_sentiment_pct,
            "top_complaint_categories": [c["category"] for c in complaints[:2]],
