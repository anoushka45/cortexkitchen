...
export default function ChatPage() {
  ...
  return (
    <main className="flex h-[calc(100vh-56px)] bg-[var(--color-surface)] relative overflow-hidden">

      {/* ── History sidebar ─────────────────────────────────────── */}
      <div className={`${historyOpen ? "w-64" : "w-0"} shrink-0 overflow-hidden border-r border-[var(--color-border-soft)] transition-all duration-200`}>
        <div className="flex h-full w-64 flex-col">
          <div className="flex items-center justify-between px-4 py-3.5 border-b border-[var(--color-border-soft)]">
            <span className="text-[10px] uppercase tracking-[0.2em] text-[var(--color-text-faint)]">Conversations</span>
            <button
              onClick={() => { startNewSession(); setInput(""); }}
              className="text-[10px] uppercase tracking-[0.15em] text-[var(--color-accent)] hover:opacity-70"
            >
              + New
            </button>
          </div>
          <div className="flex-1 overflow-y-auto py-2">
            {sessionListLoading && sessionList.length === 0 && (
              <p className="px-4 py-2 text-[11px] text-[var(--color-text-faint)]">Loading…</p>
            )}
            {!sessionListLoading && sessionList.length === 0 && (
              <p className="px-4 py-2 text-[11px] text-[var(--color-text-faint)]">No past conversations yet.</p>
            )}
            {sessionList.map((s) => (
              <button
                key={s.id}
                onClick={() => loadSession(s.id)}
                className={`block w-full truncate px-4 py-2.5 text-left text-[12px] transition-colors ${
                  s.id === sessionId
                    ? "bg-ember-500/[0.08] text-[var(--color-text-primary)]"
                    : "text-[var(--color-text-faint)] hover:bg-[var(--color-surface-raised)] hover:text-[var(--color-text-primary)]"
                }`}
              >
