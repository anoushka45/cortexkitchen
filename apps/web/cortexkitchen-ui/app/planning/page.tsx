...
const STAGES: Stage[] = [
  { key: "understand", title: "Understanding Your Situation", description: "Resolving the scenario, restaurant profile, and target date.", nodes: ["ops_manager"] },
  { key: "context", title: "Gathering Context", description: "Live signals, demand forecast, and relevant memory from past plans.", nodes: ["live_signals", "forecast", "enrichment"] },
  { key: "analyze", title: "Analyzing Your Business", description: "5 specialists working in parallel.", nodes: ["reservation", "complaint", "inventory", "market_intel", "dineout_manager"] },
  { key: "strategy", title: "Building the Strategy", description: "Combining every specialist's findings into one operational plan.", nodes: ["menu", "aggregator"] },
  { key: "review", title: "Quality Review", description: "The critic checks the plan before you see it.", nodes: ["critic"] },
  { key: "briefing", title: "Preparing Your Briefing", description: "Writing the executive summary and packaging your plan.", nodes: ["situation_summary", "final_assembler"] },
];

const TOTAL_TRACKED_NODES = STAGES.reduce((sum, s) => sum + s.nodes.length, 0);

const NODE_LABEL: Record<string, string> = {
  ops_manager: "Scenario & context",
  live_signals: "Weather, trends & regulatory notices",
  forecast: "Demand forecast",
  enrichment: "Memory search (past SOPs & plans)",
  menu: "Menu strategy",
  aggregator: "Brief assembly",
  situation_summary: "Executive briefing",
  final_assembler: "Final packaging",
};

const PARALLEL_AGENTS: { key: string; label: string; sub: string; dot: string; swiggy: boolean }[] = [
  { key: "reservation",     label: "Reservations",        sub: "Bookings & capacity",                  dot: "bg-cyan-400",    swiggy: false },
  { key: "complaint",       label: "Guest Feedback",      sub: "Complaints & sentiment",                dot: "bg-rose-400",    swiggy: false },
  { key: "inventory",       label: "Inventory",           sub: "Shortage detection",                    dot: "bg-emerald-400", swiggy: false },
  { key: "market_intel",    label: "Market Intelligence", sub: "Live Swiggy pricing & area occupancy",  dot: "bg-orange-400",  swiggy: true },
  { key: "dineout_manager", label: "Dineout Availability",sub: "Your own slot visibility tonight",      dot: "bg-orange-400",  swiggy: true },
];
    ...
    <div className={`rounded-xl ring-1 px-3.5 py-3 transition-colors ${state === "done" ? "ring-emerald-400/25 bg-emerald-500/[0.05]" : state === "running" ? "ring-[var(--color-accent)]/25 bg-[var(--color-accent)]/[0.05]" : "ring-[var(--color-border-soft)] bg-[var(--color-surface-raised)]"}`}>
      <div className="flex items-center gap-2">
        ...
        {agent.swiggy && state !== "idle" && (
          <img src="/swiggy-logo.png" alt="Swiggy" className="h-3 w-3 shrink-0 object-contain opacity-70" />
        )}
      </div>
...
function LoadingState({ completedNodes, startedNodes, nodeHints, replanCount, scenarioLabel, restaurantName }: {
  ...
  const allAgentsDone = ["reservation", "complaint", "inventory", "market_intel", "dineout_manager", "menu"].every((k) => completedNodes.has(k));
          ...
          {status === "success" && data && (
            <>
              ...
              <div className="mt-6 space-y-6">
                ...
                >
                  Review restock approvals in Action Center
