    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-[var(--color-border-soft)] bg-[var(--color-surface-raised)] px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[15px] font-bold leading-tight text-[var(--color-text-primary)]">Check Instamart Price</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="mt-3 flex-1 overflow-y-auto px-5">
          {status === "loading" && (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              ...
              <p className="text-[12px] text-[var(--color-text-faint)]">Checking Instamart…</p>
            </div>
          )}
          {status === "not_connected" && (
            <p className="py-10 text-center text-[12px] text-[var(--color-text-faint)]">Connect Swiggy to check live Instamart prices.</p>
          )}
          {status === "error" && (
            <p className="py-10 text-center text-[12px]" style={{ color: "var(--color-caution)" }}>
              Couldn&apos;t check Instamart price right now.
            </p>
          )}
          {status === "done" && results.length === 0 && (
            <p className="py-10 text-center text-[12px] text-[var(--color-text-faint)]">No matching products found on Instamart right now.</p>
          )}
          {status === "done" && results.length > 0 && (
        ...
        <div className="border-t border-[var(--color-border-soft)] bg-[var(--color-surface-sunken)] p-5">
          ...
          <button
            disabled
            title="Real Instamart checkout is blocked until Swiggy staging credentials land — this cart is a preview only."
