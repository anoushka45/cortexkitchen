...
export default function AnalyticsDetail() {
  ...
  const channelData = channelSplit && (channelSplit.dine_in_revenue + channelSplit.delivery_revenue) > 0 ? [
    { name: "Dine-in", value: channelSplit.dine_in_revenue, color: "#f7c188" },
    { name: "Delivery", value: channelSplit.delivery_revenue, color: "#e8590c" },
  ] : [];
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-border-default)] pb-0">
        ...
        <div className="mb-2 flex shrink-0 items-center gap-3">
          <Link href="/market" className="flex items-center gap-1 text-[12px] font-bold text-[var(--color-accent)]">
            See live Market Intelligence
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-6">
        <div id="business" className="card card-lift scroll-mt-20 p-6 md:col-span-3">
          <CardHeader
            title="Revenue &amp; profit trend" sub={`Last ${perfDays} days`}
            icon={<IconBusiness className="h-4.5 w-4.5" />} color={SECTION_COLOR.business}
          />
          {revenueTrend.length > 0 ? <RevenueTrendChart data={revenueTrend} /> : <EmptyState text={loaded ? "Not enough order history yet." : "Loading…"} />}
        </div>
        ...
        <div className="card card-lift p-6 md:col-span-4">
          <CardHeader
            title="Which dishes to keep, promote, or drop" sub={`Based on how well each dish sells and how much profit it earns, last ${perfDays} days`}
          />
          {allDishes.length > 0 ? <MenuEngineeringMatrix dishes={allDishes} /> : <EmptyState text={loaded ? "Not enough order history yet." : "Loading…"} />}
        </div>

        <div id="menu" className="card card-lift scroll-mt-20 p-6 md:col-span-2">
          <CardHeader
            title="Menu performance" sub={`By revenue, last ${perfDays} days`}
                ...
                <div className="space-y-1.5">
                  {topDishes.slice(0, 5).map((d, i) => (
                    <div key={d.name} className="flex items-center gap-2">
                      ...
                      <span className="truncate text-[12px] text-[var(--color-text-primary)]">{d.name}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div id="customers" className="card card-lift scroll-mt-20 p-6 md:col-span-3">
          <CardHeader
            title="Channel split" sub={`Last ${perfDays} days`}
              ...
              <div className="min-w-0 flex-1 space-y-3">
                {channelData.map((d) => (
                  <div key={d.name} className="flex items-center gap-2.5">
                    ...
                    >
                      {d.name === "Dine-in" ? <IconStorefront className="h-4 w-4" /> : <IconDeliveryBag className="h-4 w-4" />}
                    </span>
        ...
        <div className="card card-lift p-6 md:col-span-3">
          <CardHeader
            title="Complaint themes" sub={`Last ${perfDays} days`}
            icon={<IconCustomers className="h-4.5 w-4.5" />} color={SECTION_COLOR.customers}
          />
          {complaintCategories.length > 0 ? (
              ...
              {complaintCategories.slice(0, 6).map((c) => (
                <div key={c.category}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="min-w-0 flex-1 truncate text-[12px] font-medium text-[var(--color-text-soft)]">{c.category}</span>
            ...
            <p className="text-[11px] text-[var(--color-text-faint)]">{loaded ? `No complaints in the last ${perfDays} days.` : "Loading…"}</p>
          )}
        </div>

        <div id="operations" className="card card-lift scroll-mt-20 p-6 md:col-span-3">
          <CardHeader
            title="Peak hours" sub={`Avg orders/hour, last ${perfDays} days`}
        ...
        <div id="inventory" className="card card-lift scroll-mt-20 p-6 md:col-span-3">
          <CardHeader
            title="Inventory" sub="A live snapshot, not a trend — there's no historical inventory data to chart over time yet."
                    ...
                    <div key={a.ingredient} className="flex items-center gap-3 rounded-lg border border-[var(--color-border-default)] bg-[var(--color-surface-sunken)] p-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <p className="truncate text-[12.5px] font-semibold text-[var(--color-text-primary)]">{a.ingredient}</p>
