...
function findProcurementMatch(ingredient: string, output: FridayRushResponse["swiggy_procurement_options"]): { name: string; price: number; unit: string } | null {
  const options = output?.procurement_options;
  if (!options || !ingredient) return null;
  const needle = ingredient.toLowerCase();
  const match = options.find((o) => o.ingredient.toLowerCase().includes(needle) || needle.includes(o.ingredient.toLowerCase()));
  return match ? { name: match.ingredient, price: match.price, unit: match.unit } : null;
}
...
function buildSections(data: FridayRushResponse): ActionSection[] {
  ...
  if (topCritical) {
    const ingredient = String(topCritical.ingredient ?? "");
    ...
    const procurement = findProcurementMatch(ingredient, data.swiggy_procurement_options);
    doNow.push({
      title: shortfall !== null ? `Order ${shortfall}${unit} ${ingredient} immediately` : `Order ${ingredient} immediately`,
      subtitle: stock !== null ? `Critical shortage · Current stock ${stock}${unit}` : "Critical shortage",
      tag: procurement ? { label: "Swiggy Instamart", sub: `₹${procurement.price}/${procurement.unit}` } : undefined,
    });
  } else {
  ...
  if (opFocus) {
    doNow.push({ title: "Confirm operator instructions with the team", subtitle: opFocus.length > 70 ? `${opFocus.slice(0, 70)}…` : opFocus });
  }
  const topHighlight = takeStrings(menu?.highlight_items, 1)[0];
  if (topHighlight) {
    doNow.push({ title: `Feature ${topHighlight} in service brief`, subtitle: "Recommended menu focus for tonight" });
  }

  // ── Before Service ───────────────────────────────────────────────────
  const beforeService: ActionItem[] = [
    ...takeStrings(menu?.operational_notes, 2).map((t) => ({ title: t })),
    ...takeStrings(complaint?.action_items, 1).map((t) => ({ title: t })),
  ].slice(0, 3);

  // ── During Service ───────────────────────────────────────────────────
  const duringService: ActionItem[] = [];
  if (typeof reservation?.occupancy_pct !== "undefined") {
    duringService.push({ title: `Watch occupancy pacing at ${Math.round(Number(reservation.occupancy_pct))}%`, subtitle: "Keep table turns on pace" });
  }
  if (typeof reservation?.waitlist_count !== "undefined" && Number(reservation.waitlist_count) > 0) {
    duringService.push({ title: `Track waitlist movement (${reservation.waitlist_count} queued)` });
  }
  ...
  return [
    {
      ...
      callout: hasCritical
        ? { icon: "M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z", title: "This impacts tonight's service window", body: "Act now to avoid operational constraints." }
        : { icon: "M5 13l4 4L19 7", title: "Nothing urgent right now", body: "No critical action needed before service." },
    },
    {
      key: "beforeService", title: "Before Service", icon: "M12 8v4l3 2m6-2a9 9 0 11-18 0 9 9 0 0118 0z", priorityLabel: "Medium priority",
      items: beforeService,
      callout: { icon: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z", title: "Set the kitchen up for success", body: "Preparation now prevents pressure later." },
    },
    {
      key: "duringService", title: "During Service", icon: "M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z M21 12a9 9 0 11-18 0 9 9 0 0118 0z", priorityLabel: "Active focus",
      items: duringService,
      callout: { icon: "M8.288 15.038a5.25 5.25 0 017.424 0M5.106 11.856c3.807-3.808 9.98-3.808 13.788 0M1.924 8.674c5.565-5.565 14.587-5.565 20.152 0M12 20.25h.007v.008H12v-.008z", title: "Stay responsive", body: "Real-time awareness drives great service." },
    },
    {
      ...
      items: monitor,
      callout: { icon: "M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z", title: "Keep improving", body: "Monitor risks and guest sentiment." },
    },
  ];
}
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-[12.5px] font-semibold leading-snug text-[var(--color-text-primary)]">
                            <HighlightSwiggy text={item.title} />
                          </p>
                        </div>
                        {item.subtitle && (
