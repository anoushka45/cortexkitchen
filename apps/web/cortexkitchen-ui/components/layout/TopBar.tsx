...
import { getActionQueue, listRestaurantProfiles, RestaurantProfile } from "@/lib/api";
...
export default function TopBar() {
  ...
  useEffect(() => {
    if (!user) return;
    listRestaurantProfiles().then(setProfiles).catch(() => {});
              ...
              <div className="absolute left-0 top-full mt-1.5 w-56 rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-raised)] py-1.5 shadow-xl z-50">
                {profiles.map((p) => (
                  <button
                    ...
                    className="flex w-full items-center justify-between px-3 py-2 text-left text-xs text-[var(--color-text-soft)] hover:bg-[var(--color-surface-sunken)] hover:text-[var(--color-text-primary)]"
                  >
                    <span className="truncate">{p.name}</span>
                    <span className="mono ml-2 shrink-0 text-[10px] text-[var(--color-text-faint)]">{p.capacity} covers</span>
                  </button>
                ))}
              </div>
            )}
